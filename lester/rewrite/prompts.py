...
Here is the code to the entire lester Dataframe library:
...
5. Rewrite the python code such that global variables and imports are moved into the `__dataprep`` function. Make sure all imports from the original code are included!
...
Here is the entire pipeline with Data preperation, Featurisation and Model training pipeline stages. 
This is so that you can see how the entire pipeline works.

{entire_pipeline}

Here is the data preperation code to rewrite:

{code}
"""

FEATURISATION_COT_TEST = """
...
5. Rewrite the python code such that global variables and imports are moved into the ``__featurise`` function. Make sure all imports from the code to rewrite is included!
